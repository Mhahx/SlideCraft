...
- Where the user gives nothing, decide from the direction flow. The only defaults are those named as defaults in `profiles.md` and `direction.md` (the Restrained colour strategy for `read` and `update`, and the standing exit). Never fall back to an unexamined default.

## Workflow (always in this order)

1. **Brief.** One question round, following the rules in `references/direction.md` ("Ask what will change the work"). This is the only place questions are asked before building; the direction step does not open a second round unless it is in Choice mode. Missing data becomes a placeholder, never an invention. If nobody can answer, mark assumptions and go on.
...
3. **Direction** (new deck or redesign only). Follow `references/direction.md`: scene sentence, mechanism, the audience's visual world, two to three directions to choose from (Choice, the default) or one committed direction (Quick, only on explicit hand-over or a well-pinned short brief), a standing exit, and a six-block direction contract. Anything the user pinned (fonts, colours, tone words, references) stays as given. Refinement of an existing deck inherits its look and skips this step.
4. **Story.** Write the slide titles as full-sentence claims. Read them in sequence: they must carry the argument alone.
5. **Deck plan.** Write the deck plan from `references/deck-plan.md` (brief, direction, story, one deck-wide design system, slide-by-slide table) and run its consistency checks. Show it to the user. Build without waiting, unless the user asked to review it, the direction round is in Choice mode, or the plan holds an assumption only the user can decide. No slide is built before the plan exists.
