...
| Release-Workflow auch von Hand startbar (`workflow_dispatch`): er liest die Version aus `plugin.json`, legt den Tag `v<Version>` selbst an und bricht ab, wenn es das Release schon gibt | Tags lassen sich aus der Claude-Code-Sitzung nicht pushen (nur der freigegebene Branch); so entsteht das Release trotzdem, und Max kann es künftig per Klick auslösen |
| Erster Lauf des Test-Workflows auf GitHub: 88 Tests grün (5 ohne LibreOffice übersprungen), dann Abbruch: `skills-ref` aus PyPI (0.1.1) installiert seinen Befehl als `agentskills`, nicht als `skills-ref`. Version gepinnt, Befehl korrigiert, Schritt in einer frischen Umgebung nachgestellt (gültiges Paket Exit 0, Ordner ohne `SKILL.md` Exit 1) | der Workflow soll prüfen, nicht an einem Befehlsnamen scheitern |
| `docs/BRIEF.md` §11 Regel 11 geändert (Freigabe Max): verpacken mit `tools/package.py` und `agentskills validate`, Releases nur über den Release-Workflow; vorher `package_skill.py` und `quick_validate.py` aus skill-creator | seit 0.17 wurde so verpackt; die Abweichung war zuerst offen vermerkt, jetzt ist die Regel angepasst |
