...
7. **Verpacken** (erledigt in 0.17 bis 0.19): `tools/package.py` baut `dist/slide-craft.zip`, `agentskills validate` (Paket `skills-ref`) prüft gegen die Agent-Skills-Spezifikation, das Repo ist ein Plugin-Marketplace (`.claude-plugin/marketplace.json`, `plugin/`), GitHub-Workflows testen jeden PR und bauen bei jedem Versions-Tag ein Release mit ZIP. Nicht mit einem Windows-ZIP-Werkzeug packen: `Compress-Archive` erzeugt Pfade mit Backslashes.
...
11. **Verpacken** nur mit `python3 tools/package.py` (baut `dist/slide-craft.zip` aus `plugin/skills/slide-craft/`, prüft Name, Beschreibung, die in `SKILL.md` genannten Dateien und die Versionen) und danach `agentskills validate dist/slide-craft` (Paket `skills-ref` 0.1.1, Validator der Agent-Skills-Spezifikation). Releases nur über den Release-Workflow. Nicht mit einem Windows-ZIP-Werkzeug packen. (Geändert in 0.19, Freigabe Max 2026-09-25; vorher: `package_skill.py` und `quick_validate.py` aus skill-creator.)
