...
- Quote the ids of the released items in the plan's Waivers line, together with the user's words, for example: ``Waivers: `shadow` (user: "rounded, Liquid Glass")``. The check script turns exactly those items into "waived by brief"; every other item stays a finding. Every id that appears anywhere in the Waivers line counts as released, so never mention an id there that stays active ("glass-stack stays active" would release it).
- **Glass (tested in a 14-slide pitch deck):** at most one translucent panel per slide, and it marks the focus (the takeaway, the focus row, the focus milestone), never a container for every block. Light behind the glass (soft colour, a photo) as a transparent PNG whose edges fade out, never an opaque rectangle: an opaque image hides the chart next to it and shows its edges. Round the corners by a fixed radius deck-wide. Text on glass: the check script computes its contrast with the panel blended once over black and once over white, and the worse value must pass; glass on slides is therefore less transparent than in an app.
